...
//			g.fillRect(0, 0, getWidth(), getHeight());
		...
		for (int i = 0; i < pathfinding.getshopsList().size(); i++) {
			g.fillRect(pathfinding.getshopsList().get(i).getX() + 1, pathfinding.getshopsList().get(i).getY() + 1,
					size - 1, size - 1);
		}
	...
	public void MapCalculations(MouseEvent e) {
		...
		if (SwingUtilities.isLeftMouseButton(e)) {
			...
			// Otherwise, create a wall
				...
				// ********************************************
				if(clickshop == 'a') {
					Node newshop = new Node(xBorder, yBorder);
					pathfinding.addshop(newshop);
				}
		...
		// If right mouse button is clicked
			...
			// Otherwise, remove wall
				...
				Location = pathfinding.searchshop(mouseBoxX, mouseBoxY);
				if (Location != -1) {
					pathfinding.removeshop(Location);
				}
				repaint();
			}
		}
	...
	public void mouseWheelMoved(MouseWheelEvent m) {
		...
		// new X and Y values for shops
				for (int i = 0; i < pathfinding.getshopsList().size(); i++) {
					int newX = (int) Math.round((pathfinding.getshopsList().get(i).getX() * ratio));
					int newY = (int) Math.round((pathfinding.getshopsList().get(i).getY() * ratio));
					pathfinding.getshopsList().get(i).setXY(newX, newY);
				}
