...
public class APathfinding {
	...
	private ArrayList<Node> borders, open, closed, path, shops;
	...
	public void calculateNodeValues(int possibleX, int possibleY, Node openNode, Node parent) {
		...
		int HyDiff;
		int hCost=0;
		HxDiff = Math.abs(endNode.getX() - openNode.getX());
		HyDiff = Math.abs(endNode.getY() - openNode.getY());
		hCost =HxDiff + HyDiff;
		long HxDiffutil=0, HyDiffutil=0, hCostutil=0;
		for(int x=0;x < shops.size();x++) {
			HxDiffutil = Math.abs(shops.get(x).getX() - openNode.getX());
			HyDiffutil = Math.abs(shops.get(x).getY() - openNode.getY());
			hCostutil += HxDiff + HyDiff;
		}
		hCost = (int) ((0.6*hCost) +(0.4 *hCostutil)); 
		openNode.setH(hCost);
	...
	public void removeshop(int location) {
	...
	public boolean checkshopduplicate(Node node) {
		for (int i = 0; i < shops.size(); i++) {
			if (node.getX() == shops.get(i).getX() && node.getY() == shops.get(i).getY()) {
				return true;
			}
		}
		return false;
	...
	public int searchshop(int xSearch, int ySearch) {
		int Location = -1;

		for (int i = 0; i < borders.size(); i++) {
			if (shops.get(i).getX() == xSearch && shops.get(i).getY() == ySearch) {
				Location = i;
				break;
			}
		}
	...
	public ArrayList<Node> getshopsList() {
		return shops;
	}
